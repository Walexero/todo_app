...
const controlAddTodoMobile = function (renderTasks = undefined) {
  if (model.state.todo.length > 0) {
    //run if the model state is
    console.log("got ran setmobilenav");
  ...
  //   todoListComponentView.mobileRender();
  // }
  // console.log("triggereed for mobile add");
  ...
  else taskAddRenderView.mobileRender();
};
...
const controlAddTodo = function () {
  ...
  if (!todoListComponentView.getEventState()) {
...
const controlAddTask = function (task) {
  ...
  if (!todoListComponentView.getEventState()) {
    ...
    todoListComponentView.setEventState(true);
...
const controlTodoDelete = function (todoID) {
  ...
  const todo = model.deleteTodo(Number(todoID));

  //check if there's still todos left
  if (todo.length > 0) todoListComponentView.render(todo); //render todo
  else {
    model.state.currentTodo = null;
    model.state.currentTask = null;
    //runs if not mobile
    if (!mobileDeviceTrigger.matches) {
...
const controlRenderTodoAndSaveCurrentTodo = function (todoID) {
  ...
  else taskAddRenderView.render(newCurrentTodo);
};
